...
server <- function(input, output, session) {
  ...
  observeEvent(input$btn_lihat_hasil, {
    ...
    sampelku <- mapply(st_sample, peta_input()$geometry[positif], 
                       ntitik_acak[positif]) |>
      lapply(function(dat) {
        st_coordinates(dat) |>
          as.data.frame()
  ...
  output$unduh_tabel_hasil_excel <- downloadHandler(
    filename = function() {
      paste0("titik_sampel_acak_mestika_", Sys.time(), ".xlsx")
    },
  ...
  output$unduh_tabel_hasil_csv <- downloadHandler(
    filename = function() {
      paste0("titik_sampel_acak_mestika_", Sys.time(), ".csv")
    },
    content = function(file) {
  ...
  output$unduh_peta_hasil_shp <- downloadHandler(
    filename = function() {
      paste0("titik_sampel_mestika_", Sys.time(), ".zip")
    },
    content = function(file) {
  ...
  output$unduh_peta_hasil_kml <- downloadHandler(
    filename = function() {
      paste0("peta_titik_sampel_acak_mestika_", Sys.time(), ".kml")
    },
    content = function(file) {
      hasil_titik_semua_poligon <- mapply(
        st_sample, peta_input()$geometry,
  ...
  observeEvent(input$btn_bantuan, {
    ...
    showModal(modalDialog(
      ...
      a(href = "unej.id/GuideBook", "Panduan Lengkap Aplikasi Mestika", 
